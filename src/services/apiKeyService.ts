...
class ApiKeyService {
  ...
  async generateApiKey(
    ...
    return {
      apiKey: {
        ...
        permissions: JSON.parse(apiKey.permissions as string) as string[],
  ...
  async validateApiKey(key: string): Promise<{ userId: number; permissions: string[] } | null> {
    ...
    for (const apiKey of apiKeys) {
      ...
      if (isValid) {
        ...
        logger.debug(`API key validated for user ${apiKey.userId}`);
   ...
   */
  async listApiKeys(userId: number): Promise<ApiKeyResponse[]> {
    const apiKeys = await prisma.apiKey.findMany({
      where: { userId }
    });
    ...
    return apiKeys.map((key: any) => ({
      ...
      name: key.name,
      prefix: key.prefix,
      permissions: JSON.parse(key.permissions as string) as string[],
  ...
  async updateApiKey(
    ...
    return {
      ...
      permissions: JSON.parse(updatedKey.permissions as string) as string[],
