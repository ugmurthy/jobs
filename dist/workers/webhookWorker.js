...
export const initializeWebhookWorker = () => {
    ...
    const webhookWorker = new Worker("webhooks", async (job) => {
        ...
        const eventType = job.name; // Cast to valid event type
        ...
        return {
            ...
            webhooksTriggered: results.filter((result) => result === true).length,
            webhooksFailed: results.filter((result) => result === false).length
        };
